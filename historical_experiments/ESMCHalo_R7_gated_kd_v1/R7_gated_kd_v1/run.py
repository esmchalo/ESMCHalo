"""R7 development-only selection followed by one retrospective challenge evaluation."""
...
def select(overall,folds,sub):
    ...
    chosen=sorted(eligible,key=lambda arm:(ix.loc[(arm,'crossfit_Platt'),'log_loss'],-ix.loc[(arm,'crossfit_Platt'),'MCC'],-ix.loc[(arm,'raw'),'AP'],['G0'].index(arm)))[0] if eligible else 'H0'
...
def write_contrasts(overall,folds,out):
    for table,name,groupcols in [(overall,'supervision_contrasts.tsv',['mode']),(folds,'fold_supervision_contrasts.tsv',['mode','fold'])]:
        ...
        for key,g in table.groupby(groupcols):
            ...
            ix=g.set_index('arm');r=dict(zip(groupcols,key));r['contrast']='G0-H0'
            r.update({k:float(ix.loc['G0',k]-ix.loc['H0',k]) for k in ['ACC','MCC','Brier','log_loss','AP','AUROC']});rows.append(r)
...
def teacher_diagnostics(r,out):
    ...
    a=r[r.arm=='G0'].set_index('clean_id');b=r[r.arm=='H0'].set_index('clean_id').loc[a.index]
    for mode,col in [('raw','probability_uncalibrated'),('crossfit_Platt','probability_crossfit_Platt')]:
