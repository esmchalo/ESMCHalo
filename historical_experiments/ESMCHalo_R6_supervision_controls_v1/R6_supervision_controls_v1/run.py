"""R6 development-only selection followed by one retrospective challenge evaluation."""
...
def select(overall,folds,sub):
    ...
    chosen=sorted(eligible,key=lambda arm:(ix.loc[(arm,'crossfit_Platt'),'log_loss'],-ix.loc[(arm,'crossfit_Platt'),'MCC'],-ix.loc[(arm,'raw'),'AP'],['B0','M0'].index(arm)))[0] if eligible else 'H0'
...
def contrast_table(frame,by_fold=False):
    rows=[]
    for mode in ['raw','crossfit_Platt']:
        fs=range(5) if by_fold else [None]
        for fold in fs:
            part=frame[frame['mode']==mode]
            if fold is not None:part=part[part.fold==fold]
            x=part.set_index('arm')
            for label,terms in [('M0-B0',{'M0':1,'B0':-1}),('Mixup_KD-H0',{'Mixup_KD':1,'H0':-1}),('(M0-B0)-(Mixup_KD-H0)',{'M0':1,'B0':-1,'Mixup_KD':-1,'H0':1}),('H0-B0',{'H0':1,'B0':-1}),('Mixup_KD-M0',{'Mixup_KD':1,'M0':-1})]:
                row=dict(mode=mode,contrast=label)
                if fold is not None:row['fold']=fold
                for k in ['ACC','MCC','Brier','log_loss','AP','AUROC']:
                    row[k]=float(sum(sign*x.loc[arm,k] for arm,sign in terms.items()))
                rows.append(row)
    return pd.DataFrame(rows)

def write_contrasts(overall,folds,out):
    contrast_table(overall).to_csv(out/'supervision_contrasts.tsv',sep='\t',index=False)
    contrast_table(folds,True).to_csv(out/'fold_supervision_contrasts.tsv',sep='\t',index=False)
...
def main():
    ...
    try:
        ...
        if old.sha256(r5dir/'oof_predictions.tsv')!=protocol['R5_oof_sha256']:raise ValueError('R5 OOF hash mismatch')
        if old.sha256(r5dir/'PROTOCOL.json')!=protocol['R5_protocol_sha256']:raise ValueError('R5 protocol hash mismatch')
        r5status=json.loads((r5dir/'STATUS.json').read_text())
        if r5status.get('status')!='COMPLETED':raise ValueError('R5 not completed')
        oldp=json.loads((r5dir/'PROTOCOL.json').read_text())
        for key in ['seed','k2_seed_offset','student']:
            if oldp[key]!=protocol[key]:raise ValueError('R5 training setting mismatch '+key)
        r5=pd.read_csv(r5dir/'oof_predictions.tsv',sep='\t')
        r5h=r5[r5.arm=='H0'].set_index('clean_id').loc[h0.clean_id]
        np.testing.assert_allclose(r5h.student_raw_logit,h0.student_raw_logit,atol=1e-14,rtol=0)
        kd=r5[r5.arm=='Mixup'].copy()
        if len(kd)!=10567 or not kd.clean_id.is_unique:raise ValueError('R5 Mixup membership mismatch')
        check=kd.set_index('clean_id').loc[master.clean_id]
        for col in ['label','fold','homology_group']:
            if not np.array_equal(check[col].astype(str).to_numpy(),master[col].astype(str).to_numpy()):raise ValueError('R5 Mixup '+col+' mismatch')
        kd['arm']='Mixup_KD'
        old.atomic_json(out/'R5_REFERENCE.json',dict(oof_sha256=protocol['R5_oof_sha256'],protocol_sha256=protocol['R5_protocol_sha256'],retrained=False))
        frames=[h0,kd]
        for arm in ['B0','M0']:
            for f in range(5):
                old.atomic_json(out/'STATUS.json',dict(status='RUNNING',stage='training',arm=arm,fold=f));print(f'START {arm} fold={f}',flush=True)
                frames.append(tr.run_fold(arm,f,x,master,flags,protocol,device,out));print(f'DONE {arm} fold={f}',flush=True)
        oof=pd.concat(frames,ignore_index=True)
        if len(oof)!=4*10567 or oof.duplicated(['arm','clean_id']).any():raise ValueError('OOF assembly mismatch')
        old.atomic_json(out/'STATUS.json',dict(status='RUNNING',stage='development_calibration_selection'));overall,folds,sub,r=summarize(oof,master,out);write_contrasts(overall,folds,out);selection=select(overall,folds,sub);old.atomic_json(out/'SELECTION.json',selection);print('DEVELOPMENT SELECTION: '+selection['selected_arm'],flush=True)
