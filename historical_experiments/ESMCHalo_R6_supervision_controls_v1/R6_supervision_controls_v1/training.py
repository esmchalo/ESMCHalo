"""R6 train loops derived from V2-05A; same scaling, splits, optimizer, seeds and early stopping."""
from legacy_v205 import *

def training_loss(model,x,y,u,arm,rng):
    # Teacher tensor u is deliberately unused in both new branches.
    if arm=='B0':
    ...
    if arm=='M0':
        ...
        mx=lam*x+(1-lam)*x[perm];my=lam*y+(1-lam)*y[perm]
        return F.binary_cross_entropy_with_logits(model(mx),my)
    raise ValueError('Unknown R6 arm '+arm)
...
def run_fold(arm: str, fold: int, x: np.ndarray, master: pd.DataFrame, flags: pd.DataFrame,
    ...
    torch.save({"module":"R6","arm":arm,"outer_fold":fold,"inner_fold":inner,"selected_epochs":epochs,
                "model_state_dict":{k:v.detach().cpu() for k,v in model.state_dict().items()},
                "scaler_mean":sf.mean_.astype(np.float64),"scaler_scale":sf.scale_.astype(np.float64),
                "canonical_mean_only":True,"k2_fixed":False,"lora":False,"upweight":1.0,"training_variant":arm,"teacher_supervision_used":False}, ckpt)
