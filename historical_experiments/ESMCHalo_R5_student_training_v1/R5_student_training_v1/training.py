"""R5 train loops derived from V2-05A; same scaling, splits, optimizer, seeds and early stopping."""
from legacy_v205 import *

def focal_hard(logits,y,gamma=2.):
    ce=F.binary_cross_entropy_with_logits(logits,y,reduction='none')
    return ((-torch.expm1(-ce)).pow(gamma)*ce).mean()

def mix_batch(x,y,u,lam,perm):
    return (lam*x+(1-lam)*x[perm],lam*y+(1-lam)*y[perm],
            lam*torch.sigmoid(u/2)+(1-lam)*torch.sigmoid(u[perm]/2))

def training_loss(model,x,y,u,arm,rng):
    if arm=='Mixup':
        lam=float(rng.beta(.2,.2));perm=torch.as_tensor(rng.permutation(len(x)),device=x.device)
        mx,my,mq=mix_batch(x,y,u,lam,perm);z=model(mx)
        return .5*F.binary_cross_entropy_with_logits(z,my)+2*F.binary_cross_entropy_with_logits(z/2,mq)
    z=model(x)
    if arm=='Focal':
        return .5*focal_hard(z,y)+2*F.binary_cross_entropy_with_logits(z/2,torch.sigmoid(u/2))
    if arm=='H0':return loss_value(z,y,u,torch.ones_like(y),'H0')
...
def run_fold(arm: str, fold: int, x: np.ndarray, master: pd.DataFrame, flags: pd.DataFrame,
    ...
    torch.save({"module":"R5","arm":arm,"outer_fold":fold,"inner_fold":inner,"selected_epochs":epochs,
                "model_state_dict":{k:v.detach().cpu() for k,v in model.state_dict().items()},
                "scaler_mean":sf.mean_.astype(np.float64),"scaler_scale":sf.scale_.astype(np.float64),
                "canonical_mean_only":True,"k2_fixed":False,"lora":False,"upweight":1.0,"training_variant":arm}, ckpt)
