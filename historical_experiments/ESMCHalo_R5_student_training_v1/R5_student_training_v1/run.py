"""R5 development-only selection followed by one retrospective challenge evaluation."""
...
def select(overall,folds,sub):
    ...
    chosen=sorted(eligible,key=lambda arm:(ix.loc[(arm,'crossfit_Platt'),'log_loss'],-ix.loc[(arm,'crossfit_Platt'),'MCC'],-ix.loc[(arm,'raw'),'AP'],['Mixup','Focal'].index(arm)))[0] if eligible else 'H0'
...
def main():
    ...
    try:
        ...
        if len(oof)!=3*10567 or oof.duplicated(['arm','clean_id']).any():raise ValueError('OOF assembly mismatch')
        old.atomic_json(out/'STATUS.json',dict(status='RUNNING',stage='development_calibration_selection'));overall,folds,sub,r=summarize(oof,master,out);selection=select(overall,folds,sub);old.atomic_json(out/'SELECTION.json',selection);print('DEVELOPMENT SELECTION: '+selection['selected_arm'],flush=True)
